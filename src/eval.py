...
def main():
    ...
    for audio_file in sorted(project_root.glob("data/jamendolyrics/subsets/en/mp3/*.m4a"))[:1]:
    ...
    if results:
        ...
        if 'demucs_time' in df.columns:
            print(f"  Median Demucs time: {df['demucs_time'].median():.3f}s")
        if 'whisper_time' in df.columns:
            print(f"  Median Whisper time: {df['whisper_time'].median():.3f}s")
        print("="*70)
        print("\nDetailed Results:")
        print(df.to_string())
    else:
